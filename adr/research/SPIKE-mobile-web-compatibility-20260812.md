...
| ID | Priority | Evidence | Finding | Impact | Direction, not specification |
|---|---|---|---|---|---|
| MOB-005 | P1 | Measured, High | 30/32 visible plan interactives and 46/46 review interactives at 390×844 had at least one dimension below 44 px. Core controls are commonly 22–28 px high. | Mis-taps, poor one-handed use, and accessibility failure affect primary actions, not only inline links. | Add coarse-pointer hit-region primitives at the shared control layer; visual glyphs may remain compact. |
...
| MOB-029 | P1 | Physical iPhone, High | Tapping one semantic plan block—especially one bullet—selects it cleanly without click-drag precision, but there is no practical way to accumulate several adjacent or non-adjacent bullets into one annotation. | The strongest existing touch interaction stops at single-block feedback; reviewers cannot express one comment over a logical multi-item region. | Preserve tap-to-select for one block and prototype a touch-native accumulation/range model (tap to add/remove, explicit range growth, and a visible selected-block count) that never requires dragging text handles. |

### Viewport, keyboard, and platform issues

| ID | Priority | Evidence | Finding | Impact | Direction, not specification |
|---|---|---|---|---|---|
| MOB-010 | P1 | Source-confirmed platform risk, Medium | Both app roots use `h-screen`/100vh; fixed composers use `window.innerHeight/innerWidth`, not `visualViewport`. | Safari browser chrome and software keyboard can hide or displace the composer and submission actions. | Use a dynamic-viewport root and keyboard-aware sheet/composer placement. |
| MOB-011 | P1 | Measured platform risk, Medium | Shared comment textareas compute to 14 px; review annotation inputs reach 13/11 px; direct edit is 15 px. | iPhone Safari can auto-zoom focused controls, destabilizing annotation and review. | Use at least 16 px for focused editable controls on phone/coarse pointers. |
| MOB-012 | P2 | Source-confirmed platform risk, Medium | Entry viewports omit `viewport-fit=cover`; no safe-area inset use exists; bottom-fixed/mobile panels have no home-indicator padding. | Landscape notches and bottom indicators can overlap interactive chrome once edge-to-edge layouts are introduced. | Define one safe-area contract before placing bottom bars/sheets. |
...
| MOB-031 | P1 | Physical iPhone, High | The code-feedback comment box does not scale coherently to the phone viewport. The Plan comment composer was not exercised in the same pass, so parity remains unknown. | The core act of explaining a selected line is cramped or unstable even after the reviewer reaches a selectable diff. | In the Phase 1 composer checkpoint, make both code and Plan composers visible-viewport and keyboard aware, use 16 px editables, keep dismiss/submit reachable above the home indicator, and repeat the physical-device pass for both surfaces. |
...
| MOB-017 | P2 | Observed + source-confirmed, High | Review header stacks below 480 px but does not reprioritize the IA; Dockview tab chrome, Split/Unified/options, per-file controls, and header actions remain concurrent. | The shell “fits” while the review task remains cognitively overloaded. | Define a mobile review hierarchy instead of relying on wrapping. |
