...
| ID | Priority | Evidence | Finding | Impact | Direction, not specification |
|---|---|---|---|---|---|
| MOB-010 | P1 | Physical iPhone + source-confirmed; follow-up implemented, physical retest pending | Roots follow the observed visual viewport and the primary composers consume shared reactive bounds. The first iPhone pass exposed an outer-scroll mismatch: entry bodies retained `min-height: 100vh`, which can exceed Safari's visible viewport and reveal a blank page region below the app. Hook and Review entries now lock outer overflow and delegate scrolling to the application viewport. | The primary overlay geometry is bounded, but the Safari-only document-stage fix still needs physical proof; historical secondary popouts remain assigned to their owning phases. | Retest browser-chrome expansion, bottom-of-document scrolling, rotation, and keyboard transitions on Mobile Safari, then adopt the same shared bounds in later surface phases. |
