...
| ID | Priority | Evidence | Finding | Impact | Direction, not specification |
|---|---|---|---|---|---|
| MOB-010 | P1 | Physical iPhone + WebKit-confirmed; document-scroll correction implemented, physical retest pending | Roots follow the observed visual viewport and the primary composers consume shared reactive bounds. The first correction removed an outer `100vh` scroll and synchronized the Plan/card canvas, but the phone proved the remaining boundary was behavioral, not cosmetic: Plannotator's document still scrolled inside `<main>`, so Safari kept its controls expanded and stopped the visible document above them. WebKit explicitly distinguishes body scrolling, which shifts/collapses browser UI, from vertical element scrolling, which intentionally does not ([WebKit #240861](https://bugs.webkit.org/show_bug.cgi?id=240861)). Compact coarse-pointer Plan layouts now use the actual document scroller, propagate that viewport through navigation/selection consumers, and allow the page to continue behind collapsing browser controls. Desktop, Grid/HTML coloring, and Code Review retain their incumbent shell. | The known Plan shell cause is corrected in code, but Mobile Safari must prove control collapse, bottom reachability, composer geometry, and rotation together. Code Review's canvas remains a separate phase because its multi-panel workspace cannot inherit Plan's reading-page composition unchanged. | Retest upward/downward page scrolling and the true final document line with expanded and collapsed Safari controls, then repeat the composer/keyboard/rotation checks before passing 1B. |
