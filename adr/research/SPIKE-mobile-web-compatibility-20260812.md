...
| ID | Priority | Evidence | Finding | Impact | Direction, not specification |
|---|---|---|---|---|---|
| MOB-010 | P1 | Source-confirmed platform risk, Medium; Checkpoint 1A/1B implemented, physical gate pending | Roots now follow the observed visual viewport and the primary Plan / Code Review composers consume shared reactive visible bounds instead of `innerHeight` / `innerWidth`. Historical secondary popouts remain assigned to their owning phases. | Safari browser chrome and software keyboard can hide or displace any unconverted secondary composer; the primary paths now have a testable bounded contract. | Pass the primary paths on physical Mobile Safari, then adopt the same shared bounds in each later surface phase rather than adding local viewport readers. |
