...
| ID | Priority | Evidence | Finding | Impact | Direction, not specification |
|---|---|---|---|---|---|
| MOB-010 | P1 | Physical iPhone + source-confirmed; second follow-up implemented, physical retest pending | Roots follow the observed visual viewport and the primary composers consume shared reactive bounds. The first iPhone pass exposed an outer-scroll mismatch: entry bodies retained `min-height: 100vh`, which could reveal a blank page region below the app. Locking outer overflow removed that scroll, but the next pass exposed a separate paint discontinuity: the nested Plan surface used `--card` while the outer app and browser canvas used the nearly black `--background`, producing an opaque band around Safari's floating bottom controls. The active non-grid Plan now carries its card canvas through the app root and document canvas; grid, HTML, Review, loading, and desktop composition keep their incumbent backgrounds. | Overlay geometry and the known canvas mismatch are addressed, but Safari browser-chrome expansion and translucent-control composition still need physical proof; historical secondary popouts remain assigned to their owning phases. | Retest browser-chrome expansion, bottom-of-document scrolling, rotation, and keyboard transitions on Mobile Safari, then adopt the same shared bounds in later surface phases. |
