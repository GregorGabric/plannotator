...
- Worker: emit `worker.<hash>.js` as a real file. Runtime: `if (location.protocol === 'file:') → no pool (main-thread)` else create the pool; failure to construct → main-thread. Same code both ways.
...
2. **Main-thread highlighting on `file://`** for very large diffs (the deleted comment measured >2 s of `findNextMatchSync` while scrolling). Mitigation: idle-time highlight, or accept; hosted guides get the worker.
