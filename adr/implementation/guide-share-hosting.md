...
Rate limiting: Cloudflare rate-limiting binding `RATE_LIMITER` when present (`env.RATE_LIMITER.limit({ key: ip })`), no-op when the binding is absent (Bun target, tests). Uploads only.
...
- Review server endpoints (Bun `packages/server/review.ts` and Pi `apps/pi-extension/server/serverReview.ts`, both):
  - `POST /api/guide/:jobId/share` body `{ public?: boolean, ttlSeconds?: number }` → `200 { id, url, deleteToken, expiresAt?, bytes }`; `403 { error: 'sharing disabled' }`; `404` when the guide is not exportable (same resolution as `/export`); `502 { error }` on service failure. Same-origin guard as other mutating endpoints. On success, the saved guide envelope (when one exists / after `saved:` ids) records `share: { id, url, createdAt, deleteToken }` so a saved guide remembers its link.
  - `DELETE /api/guide/:jobId/share` → calls the service with the stored token, clears the envelope record; `204`; `404` no record.
  - `GET /api/guide/:jobId/share-info` → `{ enabled: boolean, serviceUrl, existing?: { url, createdAt } }`.
- CLI (`packages/server/guide/guide-cli.ts`): `plannotator guide share --id <saved> | --guide g.json --patch p.patch | --snapshot s.json [--public] [--ttl <e.g. 7d|24h|3600>] [--service-url <u>] [--json]`. stdout: the URL (or `{ id, url, deleteToken, expiresAt? }` with `--json`); stderr: `Delete with: plannotator guide unshare <id> --token <deleteToken>` and the size. `plannotator guide unshare <id> --token <t> [--service-url]` → `204` → "Removed". Exit codes as `export`: 0 / 1 (not found, invalid, service error) / 2 usage. Refuses (exit 1) when `PLANNOTATOR_SHARE=disabled`.
...
`bun run apps/guides-show/targets/bun.ts [--port 8788] [--store fs:<dir> | s3:<bucket>] [--viewer-dir dist/viewer]` serves the same routes as the Worker plus `/v1/*` from the viewer dir (same headers as the Worker: immutable, CORS `*`) and the landing. Env: `GUIDES_SHOW_STORE`, `GUIDES_SHOW_VIEWER_DIR`, `PORT`. README recipe under "Self-hosting".
