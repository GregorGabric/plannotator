...
|---|---|---|
| `POST /api/g` | JSON `{ mode: 'encrypted'\|'plain', data: string, viewer?: {…}, ttlSeconds?: number }` | `201 { id, url, deleteToken, expiresAt? }` where `url = <origin>/g/<id>` (the uploader appends `#key=…` itself for encrypted). Errors: `400` bad shape / plain that fails `parseGuideSnapshotJson` (with `path`+`message`), `413` over cap. |
...
- `worker/index.test.ts`: the wiring only — share routes dispatched through `worker.fetch` over a fake `GUIDES` bucket with the bundled manifest as the fallback pin; `/v1` asset headers; 404 pages.
