...
The diff content stays local in the base review flow. The raw patch moves from Git to the Bun server to the browser on your machine. Separately, the UI checks GitHub for the latest Plannotator release, and local Git review may run `git ls-remote origin` to identify the default branch and a stale baseline. Those requests do not contain the diff and do not give the Plannotator project owner usage analytics, although GitHub or your configured Git host receives ordinary request metadata. Ask AI, review agents, hosted PR review, sharing, and callback links have their own network behavior when you use them.
