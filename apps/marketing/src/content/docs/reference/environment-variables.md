...
| `PLANNOTATOR_SKIP_CALLDIFF_INSTALL` | off | Set to `1` / `true` / `yes` to skip installing the optional pinned CallDiff runtime used by code review's Call Flow layer. Read by the install scripts only. |
