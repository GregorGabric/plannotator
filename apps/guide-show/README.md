...
```sh
bun run --cwd apps/guide-show build:viewer          # dist/viewer
bun run --cwd apps/guide-show serve:local           # http://localhost:8787/v1/ (CDN stand-in)
bun run --cwd apps/guide-show export:samples -- --base http://localhost:8787/v1/ --out dist/samples-local
open apps/guide-show/dist/samples-local/*.html      # opens from file://, loads the local "CDN"
```
...
1. Cloudflare: create R2 bucket `guide-show-viewer`; add the `guide.show` zone; the Worker route uses `custom_domain = true`.
...
3. Run the `guide.show deploy` workflow manually once (workflow_dispatch), then it runs on every `v*` tag.
