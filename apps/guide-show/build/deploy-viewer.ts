...
/**
 * Upload a built viewer to the guide.show R2 bucket — ADD-ONLY.
 *   bun build/deploy-viewer.ts [--bucket guide-show-viewer] [--dry-run] [--local]
 *
...
const bucket = opt('--bucket', 'guide-show-viewer')!;
...
async function existingBytes(key: string): Promise<Uint8Array | null> {
  const tmp = `/tmp/guide-show-verify-${process.pid}-${Math.random().toString(36).slice(2)}`;
