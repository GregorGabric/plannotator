...
| `PLANNOTATOR_MINIMAL` | **Read by the install scripts only**, not by the runtime binary. Set to `1` / `true` / `yes` to have `scripts/install.sh` / `install.ps1` / `install.cmd` install **only** the `plannotator` binary — skipping the sem sidecar, the CallDiff runtime, the agent-terminal runtime, and all per-agent skills, hooks, slash commands, and config. Equivalent to the `--minimal` (aliased to `--binary-only`) flag; `--no-minimal` overrides it. Off by default. |
...
| `PLANNOTATOR_SKIP_CALLDIFF_INSTALL` | **Read by the install scripts only.** Set to `1` / `true` / `yes` to skip installing the optional pinned CallDiff runtime used by code review's Call Flow analysis. Off by default. |
