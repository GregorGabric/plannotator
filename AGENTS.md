...
2. **Scopes** — `defineShortcutScope({ id, title, shortcuts: { actionId: { bindings, description, section, ... } } })`. One scope per UI surface (annotation toolbar, comment popover, file tree, etc.). App-specific scopes live in `packages/ui/shortcuts/{plan-review,code-review}/` — **the subfolder names which app's UI the scope serves** — while cross-app scopes such as annotation history live at the shortcuts root. Components/Apps wire handlers to a scope via `useShortcutScope({ scope, handlers: { actionId: () => ... } })`.
...
**Annotation history ownership:** `packages/ui/shortcuts/history.shortcuts.ts` defines the shared Mod+Z / Mod+Shift+Z / Mod+Y scope; `packages/ui/utils/undoHistory.ts` and `packages/ui/hooks/useUndoHistory.ts` provide the bounded pure stack and React adapter. The editor and review apps own surface-specific commands and reset history on document/review identity changes, draft restoration, content replacement, submission, navigation, and anchor remapping; histories are never cached for an old context. Human annotation add/edit/delete operations are recorded, while external/WebMCP/editor-agent mutations clear or rebase the current stack. Native owners always win: inputs, textareas, contenteditable elements, CodeMirror/Edit Mode sessions, dialogs, and active tool overlays exclude app history. HTML/live-app frames forward history chords to the parent only when their embedded target is not a native editing owner. Image Annotator owns the same chords while its overlay is active and keeps its stroke past/future separate from document history.
