  ...
  hookModule!.useDocumentWebMcp({
    ...
    linkedDoc: {
      ...
      open: async (path) => {
        ...
        // to the caller when this promise resolves.
    ...
    fileBrowserDirs: folder ? [{ path: '/docs', tree: [
      ...
      { name: 'notes', path: 'notes', type: 'folder', children: [{ name: 'rollout.md', path: 'notes/rollout.md', type: 'file' }] },
    ] }] : [],
...
describe.skipIf(!hasDom)('useDocumentWebMcp', () => {
  ...
  test('folder sessions list the file browser tree and report its selection as the open sibling', async () => {
    ...
    expect(listed.data.documents.map((d: { path: string }) => d.path)).toEqual(['/docs/notes/rollout.md', '/docs/plan.md', '/docs/rollout.md']);
